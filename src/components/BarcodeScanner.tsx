...
export default function BarcodeScanner({ onDetected, onClose }: BarcodeScannerProps) {
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        // Önce kamera iznini iste. Bazı telefonlarda izin verilmeden
        // listVideoInputDevices() boş/yanlış sonuç döndürebiliyor.
        ...
        if (!mounted || allDevices.length === 0) {
          setError('Kamera bulunamadı. Telefonunuzda kamera olduğundan emin olun.');
          return;
        }
        setDevices(allDevices);

        // Arka kamerayı tercih et; mobil tarayıcılarda label ancak izin sonrası gelir.
        const backCamera = allDevices.find((d) => /back|rear|environment|arka/i.test(d.label)) || allDevices[0];
        const deviceId = backCamera.deviceId;
        setSelectedDeviceId(deviceId);
        startScan(deviceId);
      } catch (err) {
        console.error('Kamera hatası:', err);
        setError('Kamera açılamadı. Site HTTPS üzerinden açılmalı ve tarayıcıdan kamera izni verilmelidir.');
      }
    }
