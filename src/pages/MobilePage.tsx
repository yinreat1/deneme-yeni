...
import { Search, Plus, Minus, Package, AlertTriangle, Save, X, Barcode as BarcodeIcon, ShoppingCart, Trash2, Camera, BarChart3 } from 'lucide-react';
...
type MobileTab = 'manage' | 'sell' | 'reports';
    ...
    <div className="flex h-screen flex-col overflow-hidden bg-slate-50">
      ...
      <div className="flex bg-white border-b border-slate-200">
        ...
        <button
          ...
          className={`flex-1 py-3 text-sm font-medium transition-colors ${
            tab === 'reports' ? 'text-teal-700 border-b-2 border-teal-600' : 'text-slate-500'
          }`}
        >
      ...
      <div className="flex-1 overflow-hidden">
        {tab === 'manage' ? <ManageTab /> : tab === 'sell' ? <SellTab /> : <ReportsPage />}
      </div>
    </div>
  );
}
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Ürün ara veya barkod yaz..."
              value={search}
          ...
          <button
            ...
            title="Telefon kamerasıyla barkod okut"
          >
            <Camera size={19} />
          </button>
        </div>
        <p className="mt-1 text-[11px] text-slate-400">Barkodu okutunca kayıtlı ürünün bilgileri açılır; düzenleyip kaydedebilirsin.</p>
      </div>
...
function MobileProductForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(false);
    if (!ok) {
      setError('Ürün kaydedilemedi. Veritabanı bağlantısını kontrol edin.');
      return;
    }
    setSaved(true);
